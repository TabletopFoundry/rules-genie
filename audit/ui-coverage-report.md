...
- **Remediation #1:** Added requested-game normalization plus visible fallback notices for unsupported shared links on `/ask` and `/quick-start` (`src/app/ask/page.tsx`, `src/components/chat-interface.tsx`, `src/components/quick-start-explorer.tsx`, `src/lib/ux.ts`).
- **Remediation #2:** Added a persistent assistant context block that explains demo/live/fallback behavior and clarifies local per-game session memory (`src/components/chat-interface.tsx`, `src/app/ask/page.tsx`).
- **Remediation #3:** Added active-filter chips, reset actions, a richer result summary, and no-results recovery to the game library (`src/components/library-browser.tsx`, `src/lib/ux.ts`).
- **Remediation #4:** Added dashboard quick-action launchers plus stronger re-entry links for recent questions, saved answers, and empty states (`src/components/dashboard-client.tsx`).
- **Remediation #5:** Made answer feedback progressive, parsed the success payload, and added visible confirmation after submission (`src/components/feedback-controls.tsx`, `src/lib/api-schemas.ts`).
- **Remediation #6:** Added regression coverage for deep-link normalization, mode description, and library-filter logic plus a runnable `npm test` script (`tests/ux.test.ts`, `tsconfig.tests.json`, `package.json`, `src/lib/utils.ts`).

### Deferred

- None. All planned remediations from Phase 4 were implemented in Phase B.
