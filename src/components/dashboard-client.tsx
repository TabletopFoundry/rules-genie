...
export function DashboardClient({ initialSnapshot, games }: { initialSnapshot: DashboardSnapshot; games: GameRecord[] }) {
    ...
    <div className="space-y-8">
      <section className="grid gap-6 lg:grid-cols-[1.2fr_1fr]">
        <div className="rounded-[32px] border border-board-forest/10 bg-white p-6 shadow-card">
          ...
          <h2 className="mt-3 text-3xl font-bold text-board-pine">Welcome back, {snapshot.profile.name.split(' ')[0]}.</h2>
          <p className="mt-3 max-w-2xl text-slate-600">
            Your dashboard keeps a lightweight local history in SQLite. Use it to manage your collection, revisit recent rulings, and save the answers you want at the table.
          </p>
          <div className="mt-6 flex flex-wrap gap-3 text-sm">
            <span className="rounded-full bg-board-mist px-4 py-2 font-semibold text-board-pine">{snapshot.profile.email}</span>
            <span className="rounded-full bg-board-gold/20 px-4 py-2 font-semibold text-board-pine">{snapshot.profile.mode}</span>
          </div>
        </div>
      ...
      <section className="rounded-[32px] border border-board-forest/10 bg-white p-6 shadow-card">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          {!snapshot.collection.length ? <div className="rounded-[28px] border border-dashed border-board-forest/20 p-8 text-sm text-slate-500">No games saved yet. Add one above to build your library.</div> : null}
        </div>
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-[32px] border border-board-forest/10 bg-white p-6 shadow-card">
          ...
          <div className="mt-5 space-y-4">
            ...
            {!snapshot.recentQuestions.length ? <p className="text-sm text-slate-500">Ask a rules question to see it here.</p> : null}
          </div>
        </div>
        ...
        <div className="rounded-[32px] border border-board-forest/10 bg-white p-6 shadow-card">
          ...
          <div className="mt-5 space-y-4">
            ...
            {!snapshot.bookmarks.length ? <p className="text-sm text-slate-500">Bookmark answers from the chat screen to keep them handy.</p> : null}
          </div>
        </div>
      </section>
    </div>
